...
describe.skip('toBeDisabled()', () => {
  ...
  describe('normal', () => {
    it('passes when disabled', () => {
      expect(async () => {
        runMatcher(toBeDisabled((await render(<input disabled />)).root));
      }).not.toThrowError();
    });

    it('errors when not disabled', () => {
      expect(async () => {
        runMatcher(toBeDisabled((await render(<input />)).root));
      }).toThrowError(
  ...
  describe('negated', () => {
    it('passes when not disabled', () => {
      expect(async () => {
        runMatcher(toBeDisabled((await render(<input />)).root), true);
      }).not.toThrowError();
    });

    it('errors when disabled', () => {
      expect(async () => {
        runMatcher(toBeDisabled((await render(<input disabled />)).root), true);
      }).toThrowError(
