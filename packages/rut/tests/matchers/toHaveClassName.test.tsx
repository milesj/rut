...
describe.skip('toHaveClassName()', () => {
  ...
  describe('normal', () => {
    it('passes for exact class name', () => {
      expect(async () => {
        runMatcher(toHaveClassName((await render(<div className="foo" />)).root, 'foo'));
      }).not.toThrowError();
    });

    it('passes when one of the class names exist', () => {
      expect(async () => {
        runMatcher(toHaveClassName((await render(<div className="foo bar baz" />)).root, 'bar'));
      }).not.toThrowError();
    });

    it('errors when the class name does not match', () => {
      expect(async () => {
        runMatcher(toHaveClassName((await render(<div className="foo bar" />)).root, 'baz'));
  ...
  describe('negated', () => {
    it('errors for exact class name', () => {
      expect(async () => {
        runMatcher(toHaveClassName((await render(<div className="foo" />)).root, 'foo'), true);
    ...
    it('errors when one of the class names exist', () => {
      expect(async () => {
        runMatcher(
          toHaveClassName((await render(<div className="foo bar baz" />)).root, 'bar'),
          true,
        );
    ...
    it('passes when the class name does not match', () => {
      expect(async () => {
        runMatcher(toHaveClassName((await render(<div className="foo bar" />)).root, 'baz'), true);
      }).not.toThrowError();
    });
  });
});
